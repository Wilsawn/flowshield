...
const FilePreviewCard: React.FC<FilePreviewCardProps> = ({ file, onRemove }) => {
    ...
    return (
        <div className={`relative group flex-shrink-0 w-24 h-24 rounded-xl overflow-hidden border border-claude-bg-300 bg-claude-bg-200 animate-fade-in transition-all hover:border-claude-text-400`}>
            {isImage ? (
    ...
    return (
        <div className="relative group flex-shrink-0 w-28 h-28 rounded-2xl overflow-hidden border border-claude-bg-300 bg-claude-bg-100 animate-fade-in p-3 flex flex-col justify-between shadow-sm">
            ...
            <div className="flex items-center justify-between w-full mt-2">
                <div className="inline-flex items-center justify-center px-1.5 py-[2px] rounded border border-claude-bg-300 bg-claude-bg-100">
                    <span className="text-[9px] font-bold text-claude-text-400 uppercase tracking-wider font-sans">PASTED</span>
                </div>
            </div>
        ...
        <div className="relative" ref={dropdownRef}>
            ...
            {isOpen && (
                <div className="absolute bottom-full right-0 mb-2 w-[260px] bg-claude-bg-100 border border-claude-bg-300 rounded-2xl shadow-2xl overflow-hidden z-50 flex flex-col p-1.5 animate-fade-in origin-bottom-right">
                    {models.map(model => (
                        ...
                        >
                            <div className="flex flex-col gap-0.5">
                                <div className="flex items-center gap-2">
                                    <span className="text-[13px] font-semibold text-claude-text-100">
                                        {model.name}
                                    </span>
                                    {model.badge && (
                                ...
                                <span className="text-[11px] text-claude-text-300">
                                    {model.description}
                                </span>
                            </div>
                    ...
                    <div className="h-px bg-claude-bg-300 my-1 mx-2" />

                    <button className="w-full text-left px-3 py-2.5 rounded-xl flex items-center justify-between group transition-colors hover:bg-claude-bg-200 text-claude-text-100">
                        <span className="text-[13px] font-semibold">More models</span>
                        <Icons.SelectArrow className="w-4 h-4 -rotate-90 text-claude-text-300" />
                    </button>
                </div>
            )}
        </div>
