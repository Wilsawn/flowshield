...
const SYSTEM_PROMPT = `You are the FlowShield Builder Copilot — an expert assistant for developers integrating privacy-preserving compliance into DeFi protocols on the Flow blockchain.
...
- Regulations: MiCA (EU), FinCEN/BSA (US), FCA (UK), MAS (Singapore), FINTRAC (Canada)
- The key integration pattern: import ComplianceAction, call verify(address) before financial operations
...
When giving code examples, use Cadence 1.0 syntax. Be concise and practical. Focus on what the developer needs to ship.`
...
export async function chat(userMessage, conversationHistory = []) {
  ...
  if (apiKey) {
    try {
      ...
      console.log('[Copilot] Claude API error, using fallback:', err.message)
    }
  }
